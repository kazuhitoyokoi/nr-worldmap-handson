...
- スマホのセンサや飛行機の位置など、身近で面白いデータを扱う
- ソフトウェアエンジニアが扱う条件判定、代入操作、変数、デバッガ、音声合成、GitHub、MQTTなどの技術を知る
- 生成AIによって簡単にプログラムを開発できることを体験する
...
約1分後、このGitHub Codespacesの環境は、自動的に必要なソフトウェアをダウンロードした後、Node-REDを起動します。
...
1. パレット（左側）
   - ここには、利用可能なノードの一覧が表示されます。ノードは、特定の機能を持つ部品であり、ドラッグ＆ドロップでフローエディタに追加できます。
2. ワークスペース（中央）
...
injectノードは、フローを開始するノードです。injectノードをワークスペースに上に置くと名前が「タイムスタンプ」に変わります。templateノードは、定型文のメッセージを記入するノードです。play audioノードは音声を再生するノードです。

## 2. templateノードの設定
まず、templateノードをダブルクリックして、プロパティ設定画面を開きます。プロパティ設定画面ではノード固有の設定を行えます。templateノードのプロパティでは、テキストエディタの部分に読み上げたい文章として「Hello」と入力して、「完了」ボタンをクリックします。
...
## 3. 音声の再生
「タイムスタンプ」という名前が付いているinjectノードの左側のボタンをクリックすると、ブラウザから「Hello」という音声が再生されます。

<img width="900" border="1" src="images/play_audio.png">

もし再生されない場合は、play audioノードのプロパティを開き、「TTSボイス」のプルダウンメニューから他の音声合成エンジンを選択してみてください(例えば日本語のGoogle Chromeブラウザの場合は「Google 日本語 (ja-JP)」を選択します)。また、PCのボリュームが小さくないかも確認してみてください。
...
## 1. フローデバッガの有効化
右下にあるアイコン一覧から「フローデバッガ」をクリックします。その後「無効」スイッチをクリックして「有効」に切り替えます。
...
## 3. メッセージ内容の確認
injectノードの左側のボタンをクリックすると、ワイヤーを流れるメッセージがブレイクポイントで一時停止します。右側のフローデバッガのエリアにメッセージの内容が表示されます。
...
これで、ワイヤーを流れるメッセージの内容を確認する方法がわかりました。フローを流れるメッセージを確認しない時は、フローデバッガのエリアにある「有効」スイッチをクリックして「無効」に切り替えます。
...
スクリーンショットの画像には、前に作成したフローは存在しないですが、残しておいても問題ありません。
...
「サーバ」の鉛筆アイコンの隣にあるプラスボタンをクリックして、新しいMQTTブローカーの設定を追加します。
...
templateノードは、定型文のメッセージを記入するノードです。templateノードのプロパティには読み上げたい文章として「エラー値」と記載します。

<img width="900" border="1" src="images/template_abnormal.png">

## 4. デプロイボタンを押してフローを実行
デプロイボタンを押してから、MQTTブローカーに加速度センサのZ軸の値が10を超えるメッセージを送信すると、「エラー値」という音声が再生されます。
...
# 空港に到着したら音声で通知するフロー
ここでは、スマホのGPS情報をもとに空港に到着したら音声で通知するフローを作成します。
...
ご自身のスマホで試したい場合は、[位置情報送信機能付きIoT PhoneアプリのURL](https://kazuhitoyokoi.github.io/iot-phone/gps.html)にアクセスして、トピック名として一意の名前(nodered451など)を入力して、位置情報データを送信することもできます。ただし、インターネット上に位置情報が公開されてしまうため、ご自宅などではアクセスしないようにしてください。
